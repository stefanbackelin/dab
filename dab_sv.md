...
#### 2. Kompetenschef / Personalchef (Chapter Lead / People Manager)
...
* **Huvudansvar:** Maximera värdet av det arbete teamet utför. Äger prioriteringen av `🔵 Refinement Queue` och driver strategiska beslut under *Horizon Sync*.
...
* **Vem kan ha rollen:** Vilken utvecklare (Software Engineer) som helst i teamet. Rollen roterar organiskt från Epic till Epic. En AI-agent kan *aldrig* ha denna roll.
...
* **Vem kan ha rollen:** Utvecklare i teamet, eller autonoma AI-agenter (t.ex. GitHub Copilot Workspace, Devin).
...
För att säkerställa långsiktig stabilitet och förhindra att systemet degraderas över tid måste implementationen strikt följa fyra grundläggande operationella ramvillkor.
...
Ibland upptäcker teamet mitt i utvecklingsfasen och exekveringen att en Epic är betydligt större eller mer komplex än vad som framkom under Refinement-mötet.
...
2. **Radikalt minskad mötestid:** Utvecklingsteamet slipper sitta med på abstrakta intagsmöten. De kliver först in när en Epic har fått en tilldelad Driver och ska förfinas under ett Refinement-möte.
...
Modellen är arkitektoniskt förberedd för att integrera AI-agenter (t.ex. GitHub Copilot Workspace) i utvecklingsflödet. Systemets strikta struktur med `Epic Drivers` och `Tasklists` gör att agenter kan verka autonomt utan att skapa kodkaos.
...
Hitta standardfältet `Status` under "Fields" i projektets inställningar och mappa om kolumnerna till följande fyra exakta värden:
...
### Steg 2b: Sätt WIP-gränser (Work in Progress)
...
### Steg 4: Konfigurera vyerna (Utveckling, Ledning & Personalansvar)
...
* **C. People & Accountability Audit (För Kompetens- och Linjechefer):**
  ...
  * Sätt filter till: `is:issue label:epic`.
  * Välj **Slice by**: `Assignee` (eller ditt anpassade `Driver`-fält), alternativt **Swim lanes**: `Assignee`.
